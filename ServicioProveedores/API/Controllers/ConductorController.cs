...
public async Task<IActionResult> GetConductorIdBySub(string sub)
{
    ...
    public async Task<IActionResult> UpdateActivo(Guid id, [FromBody] bool activo)
    {
        var conductor = await _repository.GetByIdAsync(id);
    ...
    [HttpPut("{id}/estado")]
    public async Task<IActionResult> ActualizarEstado(Guid id, [FromQuery] bool estado)
    {
        var conductor = await _repository.GetByIdAsync(id);

        if (conductor == null)
        {
            return NotFound("El conductor no existe.");
        }

        conductor.CambiarEstado(estado); // Cambia el estado (true = disponible, false = ocupado)
        await _repository.UpdateAsync(conductor);

        return NoContent(); // Retorna 204 indicando que se realizó correctamente
    }
}
}
